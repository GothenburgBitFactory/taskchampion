...
mod test {
    ...
    async fn test_sync() -> Result<()> {
        ...
        let mut storage1: InMemoryStorage = StorageConfig::InMemory.try_into().unwrap();
        storage1
            .txn({
        ...
        let mut storage2: InMemoryStorage = StorageConfig::InMemory.try_into().unwrap();
        storage2
            .txn({
                let server = server.clone();
                move |txn| {
    ...
    async fn test_sync_create_delete() -> Result<()> {
        let server = TestServer::new();
        let storage1: InMemoryStorage = StorageConfig::InMemory.try_into().unwrap();
        storage1
            .txn({
                let server = server.clone();
                move |txn| {
        ...
        let storage2: InMemoryStorage = StorageConfig::InMemory.try_into().unwrap();
        storage2
            .txn({
                let server = server.clone();
                move |txn| {
    ...
    async fn test_sync_avoids_snapshot() -> Result<()> {
        let storage: InMemoryStorage = StorageConfig::InMemory.try_into().unwrap();
