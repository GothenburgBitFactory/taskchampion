...
impl SnapshotTasks {
    ...
    pub(super) fn decode(snapshot: &[u8]) -> eyre::Result<Self> {
...
/// storage.
pub(super) fn make_snapshot(txn: &mut dyn StorageTxn) -> eyre::Result<Vec<u8>> {
...
pub(super) fn apply_snapshot(
    ...
    if !txn.is_empty()? {
        eyre::bail!("Cannot apply snapshot to a non-empty task database");
    }
...
mod test {
    ...
    #[test]
    fn test_serialize_empty() -> eyre::Result<()> {
    ...
    #[test]
    fn test_serialize_tasks() -> eyre::Result<()> {
        let u = Uuid::new_v4();
